...
def sync_table(src_cfg, tgt_cfg, table_name, chunk_size, batch_insert_size, resume_mode=False, task_id=None, stop_flag=None):
    ...
    try:
        ...
        while current_start <= max_pk:
            if stop_flag and stop_flag["stop"]:
                logger.info("Sync stopped by user for table: %s", table_name)
                db2_store.mark_progress_failed(table_name, "用户停止")
                return {"table": table_name, "rows": synced_rows, "time": time.time() - start_time, "error": "stopped by user"}
...
def run_sync(config, resume_mode=False, restart=False, task_id=None, stop_flag=None):
    ...
    db2_store.update_task(task_id, status="running", started_at=time.strftime("%Y-%m-%dT%H:%M:%S"), message="同步任务正在执行...")

    with ThreadPoolExecutor(max_workers=parallel) as executor:
        futures = {}
        for tbl in tables:
            future = executor.submit(
                sync_table, src_cfg, tgt_cfg, tbl, chunk_size, batch_insert_size, resume_mode, task_id, stop_flag,
            )
            futures[future] = tbl
    ...
    if errors:
        has_stop = any("stopped by user" in e["error"] for e in errors)
        if has_stop:
